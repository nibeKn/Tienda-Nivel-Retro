    ...
    <div class="n64-content">
      ...
      <div class="shelf-wrapper">
        <div class="n64-shelf">
          ...
          <div class="shelf-header">
            <img src="../assets/img/Logo N64.png" alt="N64 Logo" class="shelf-logo" />
            <h2 class="shelf-title">NINTENDO<sup>64</sup></h2>
          </div>
      ...
      <aside class="n64-info-panel">
        <div class="info-card">
              ...
              <div class="media-gallery" v-if="selectedItem.media && (selectedItem.media.box3d || selectedItem.media.cart)">
                ...
                <div class="gallery-grid">
                  ...
                  <div class="gallery-item" v-if="selectedItem.media.cart">
                    <img :src="selectedItem.media.cart" alt="Cartridge Front" @click="openLightbox(selectedItem.media.cart)" class="clickable-img" />
                    <span class="gallery-label">Cartucho</span>
                  </div>
                </div>
              </div>
              ...
              <div class="console-gallery" v-if="selectedItem.isHardware && selectedItem.gallery">
                ...
                <div class="console-gallery-grid">
                  <div class="console-gallery-item" v-for="(imgSrc, index) in selectedItem.gallery" :key="index">
                    <img :src="imgSrc" :alt="`N64 Console Image ${index + 1}`" @click="openLightbox(imgSrc)" class="clickable-img" />
                  </div>
                </div>
              </div>
          ...
          <!-- Vista Global N64 si no hay selección -->
          <template v-else>
            <h2 class="info-title">Nintendo 64</h2>
            <p class="info-desc">
              La Nintendo 64 (comúnmente abreviada como N64) es una videoconsola de sobremesa de quinta generación producida por Nintendo. Su desarrollo representó el gran salto de la compañía hacia los entornos tridimensionales (3D). Fue pionera en la industria al popularizar el uso del joystick analógico mediante su peculiar mando con forma de tridente, lo que permitió un control de cámara y movimiento preciso en títulos revolucionarios que sentaron las bases del diseño 3D moderno.
            </p>
  
            <h3 class="info-subtitle">Detalles técnicos</h3>
            <p class="info-text">
              A nivel de hardware, la consola destacaba por su unidad central de procesamiento de 64 bits, desarrollada en conjunto con Silicon Graphics. A diferencia de sus principales competidores, Nintendo optó por mantener el formato de cartucho ROM (Game Pak). Esta decisión garantizaba tiempos de carga prácticamente nulos y disuadía la piratería, pero limitaba drásticamente la capacidad de almacenamiento (hasta 64 MB), lo que dificultaba la inclusión de texturas de alta resolución, audio de calidad CD y secuencias de video pre-renderizadas.
            </p>
  
            <div class="info-footer">
              <p><strong>Año de lanzamiento:</strong> 1996</p>
              <p><strong>Unidades vendidas:</strong> 32,93 millones</p>
            </div>
          </template>
        </div>

        <!-- Botones N64 con Aletas -->
        <div class="n64-tabs-container" v-if="selectedItem">
          ...
          >
            <div class="n64-btn btn-a" :class="{ pressed: !showDetailsView }">
          ...
          <div 
            ...
            title="Ficha Técnica"
          >
            <div class="n64-btn btn-b" :class="{ pressed: showDetailsView }">
...
/* Background Pattern (Nintendo 64 repitiendose) */
.pattern-bg {
  position: absolute;
  top: -100vh;
  left: -100vw;
  width: 300vw;
  height: 300vh;
  background-image: url("data:image/svg+xml,%3Csvg width='220' height='120' viewBox='0 0 220 120' xmlns='http://www.w3.org/2000/svg'%3E%3Ctext x='50%25' y='50%25' dy='.3em' font-family='Arial, sans-serif' font-size='26' font-weight='800' fill='%23cbd5e1' text-anchor='middle'%3ENINTENDO 64%3C/text%3E%3C/svg%3E");
...
/* -- Botones de Control N64 y Aletas -- */
.n64-tabs-container {
  position: absolute;
  top: 90px; 
...
.btn-a .btn-letter {
  color: #123e8f; /* Relieve oscuro tallado */
...
/* Top Banner matching the N64 image */
.shelf-header {
  ...
  justify-content: center;
  gap: 25px;
  border: 5px solid #23853b;
  border-bottom: none;
  box-shadow: inset 0 12px 20px rgba(255,255,255,0.25), inset 0 -10px 15px rgba(0,0,0,0.15);
}

.shelf-logo {
  height: 110px; /* Agrandado de 80px */
...
.shelf-title {
  ...
  margin: 0;
  letter-spacing: 1px;
  text-shadow: 2px 2px 0px #b1c1ce, 4px 4px 6px rgba(0,0,0,0.4);
}
...
.shelf-body {
  flex: 1;
  background-color: #267136; /* Darker green inside */
  border-left: 20px solid #349c4f; /* Side walls */
  border-right: 20px solid #349c4f;
  border-top: 15px solid #1c572a; /* Shadow directly under header */
...
.shelf-row {
  flex: 1;
  border-bottom: 32px solid #349c4f; /* Physical shelf floor */
...
.shelf-row:last-child {
  border-bottom: 50px solid #349c4f; /* Base of the stand is thicker */
